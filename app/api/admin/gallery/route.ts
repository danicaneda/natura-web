...
export async function GET() {
  ...
  try {
    const res = await fetch(`${B}/api/admin/gallery`, { headers: auth(), cache: 'no-store' });
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const res = await fetch(`${B}/api/admin/gallery`, { method: 'POST', headers: auth(), body: JSON.stringify(body) });
    return NextResponse.json(await res.json(), { status: res.status });
  } catch { return NextResponse.json({ error: 'Backend no disponible' }, { status: 503 }); }
}
