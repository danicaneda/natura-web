...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const res = await fetch(`${B}/api/admin/productos/${id}`, { method: 'PUT', headers: auth(), body: JSON.stringify(body) });
...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  try {
    const res = await fetch(`${B}/api/admin/productos/${id}`, { method: 'DELETE', headers: auth() });
    return NextResponse.json(await res.json(), { status: res.status });
  } catch { return NextResponse.json({ error: 'Backend no disponible' }, { status: 503 }); }
}
